...
async function initSession(modelUrl, wasmDir) {
  ...
  try {
    ...
    // Attempt WebGPU first for sub-15ms acceleration, fall back to multi-threaded WASM SIMD
    try {
      session = await ort.InferenceSession.create(modelBytes, {
        executionProviders: ["webgpu", "wasm"],
        graphOptimizationLevel: "all"
      });
      console.log("[Worker] Initialized with execution provider: WebGPU / WASM");
    } catch (gpuErr) {
      ...
      console.log("[Worker] Initialized with execution provider: WASM SIMD");
    }
